...
?><?php
		if (isset($_POST['edit_nooemsubmit'])){
		...
		// валидации
			...
			validate_min_lengths($fields_with_min_lengths);
				...
				$crossttc_art_id = mysql_prep($_POST["crossTTC_ART_ID"]);
				$contained1 = mysql_prep($_POST["contained1"]);
				

				$safe_brand = mysqli_real_escape_string($connection,$_SESSION["brand"]);
				
				// 2. Perform database query
				$query  = "UPDATE {$safe_brand}_nooem SET";
				...
				$query .= " crossTTC_ART_ID = '{$crossttc_art_id}',";
				$query .= " contained1 = {$contained1}";
				...
				if ($result && mysqli_affected_rows($connection) == 1) {
					// Success
					$_SESSION["message"] = "NoOem stroka updated.";
					//redirect_to("manage_content.php");		
				} else {
					// Failure
					$_SESSION["message"] = "NoOem stroka update failed."."(SQL state: ".mysqli_sqlstate($connection)."). Error: ".mysqli_errno($connection).". ".mysqli_error($connection);				
				}
				...
				$oemstroka = array("NAME_PARTS"=>$name_parts, );*/
			...
			if (isset($_POST['new_nooemsubmit'])){
			// Если отправлено, то обработка формы
			...
			validate_min_lengths($fields_with_min_lengths);
					...
					// 2. Perform database query
					$query  = "INSERT INTO {$safe_brand}_nooem (";
					$query .= " NAME_PARTS,";
					...
					$query .= " crossTTC_ART_ID,";
					$query .= " contained1";
					$query .= ") VALUES (";
					$query .= "  '{$name_parts}', '{$mainart_brands}', '{$mainart_code_parts}', '{$ttc_art_id}', '{$brands}', '{$code_parts}', '{$code_parts_advanced}', '{$crossttc_art_id}', {$contained1}";
					$query .= ")";
					...
					if ($result && mysqli_affected_rows($connection) >= 0) {
						// Success
						$_SESSION["message"] = "NoOem stroka created.";
						//redirect_to("manage_content.php");		
					} else {
							// Failure
							$_SESSION["message"] = "NoOem stroka creation failed."."(SQL state: ".mysqli_sqlstate($connection)."). Error: ".mysqli_errno($connection).". ".mysqli_error($connection);				
					}
...
?><?php 
	...
	function find_new_nooemstroku($brands, $code_parts, $mainart_code_parts) { // Эта функция работает только для уже созданных строк. Для той, которую только пытаемся создать, но например, не смогли, ничего не будет возвращено . 
		global $connection;
		global $query2;
		...
		$query2  = "SELECT * ";
		$query2 .= "FROM {$safe_oembrand}_nooem ";
		$query2 .= "WHERE BRANDS = '{$brands}' AND ";
		$query2 .= " CODE_PARTS = '{$code_parts}' AND ";
		$query2 .= " mainART_CODE_PARTS = '{$mainart_code_parts}' ";
		$query2 .= "LIMIT 1";
		$newnooem_set = mysqli_query($connection, $query2);
		// Test if there was a query error
		 //ec = "<div id=\"form\">".$query2."<hr/><div id=\"form\">";
		 
		 
		if (!$newnooem_set){
		die("NOT newnooem_set in the func(find_new_nooemstroku).Database do not returned anything.");}
		...
		if ($oemstroka = mysqli_fetch_assoc($newnooem_set)) {
			return $oemstroka;
		} else {
			return null;
		}
...
<?php   
	...
	} elseif (isset($_POST['new_nooemsubmit'])) {  // Иначе, если получили POST new_oemsubmit - получаем значения 
		if (empty($errors)){ // если нет ошибок - из новой строки
		$oemstroka = find_new_nooemstroku($brands, $code_parts, $mainart_code_parts); //В этой функции делается SELECT, чтобы сделать выбор получившейся строки и в итоге получить массив $oemstroka для предзаполнения полей в форме. 
			
			} else { // а если есть ошибки - из существующего в полях id
				$oemstroka = find_nooemstroku_by_POST_id();
			}  
		//print_r($oemstroka);
	} else { // Иначе, если это был POST edit_oemsubmit
		
		if (isset($_POST['edit_nooemsubmit'])) { // Получаем значения из имеющегося в полях post id
			$oemstroka = find_nooemstroku_by_POST_id();
		}
...
?>
	<form id="edit_nooemnumber" name="edit_nooemnumber" method="post" action="edit_nooemnumber.php">
			<br/>
			  ...
			  crossTTC_ART_ID:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			  <input type="text" name="crossTTC_ART_ID" id="crossTTC_ART_ID" value="<?php echo htmlentities($oemstroka["crossTTC_ART_ID"])?>"/><br/><br/>
			  
			  contained1:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			   <select name="contained1">
			   
			   <option value="0" 
			   <?php if ($oemstroka["contained1"] == 0 || is_null($oemstroka["contained1"])) {?> 
			   selected
			   <?php } ?>
			   >0</option> 
			  			   
			   <option value="1"
			   <?php if ($oemstroka["contained1"] == 1) { ?>
			   selected
			   <?php } ?>
			   >1</option>
			  ...
			  <br/><br/>
			  <input type="submit" name="edit_nooemsubmit" id="edit_nooemsubmit" onclick="return confirm('Редактировать этот?');" value="Редактировать этот" /><br/><br/>
			  <input type="submit" name="new_nooemsubmit" id="new_nooemsubmit" value="Создать Новый" /><br/><br/>
